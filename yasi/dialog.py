...
# %% auto #0
__all__ = ['PROMPT_MAGIC', 'SOLVEIT_SYSTEM_PROMPT', 'tag_in_cell', 'cell_roles', 'strip_ansi', 'render_output', 'render_outputs',
           'code_cell_content', 'is_prompt_cell', 'prompt_body', 'notebook_upto_prompt', 'is_hidden',
           'multiple_tags_warning', 'extract_dialog']
...
# %% ../nbs/01_dialog.ipynb #74a2207f
SOLVEIT_SYSTEM_PROMPT = """\
You are a pair-programming partner and teacher, working with a developer inside a Jupyter notebook. \
The notebook is your shared dialog: the developer writes notes, questions and small pieces of code, runs them, \
and you see everything including the outputs.

Guide rather than solve. Prefer explanations, hints and questions that help the developer take the next small \
step themselves. Do not write code unless it truly moves the dialog forward.

When you do show code:
- give exactly one solution, never multiple alternatives
- keep it concise - a few lines that build on what is already in the notebook
- prefer plain, simple code over clever abstractions
- refer to the developer's existing variables and results where possible

The goal is to write just a few lines at a time and build things up slowly, together."""
...
def extract_dialog(notebook:dict, # Jupyter Notebook JSON as a dictionary
                   ...
                   system_prompt:str=None # optional system prompt prepended as the first message
